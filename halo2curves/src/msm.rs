...
pub fn vesta_specialized_msm(
    ...
    if bases.len() < 32 {
        return msm_best_internal(coeffs, bases);
    }

    // For big MSMs we use pasta-msm
    unsafe {
        // Cast the halo2curves types to the types used by pasta-msm (the types from pasta-curves)
        let vesta_coeffs: &[pasta_curves::Fp] = std::mem::transmute(coeffs);
        let vesta_bases: &[pasta_curves::EqAffine] = std::mem::transmute(bases);

        // Call msm code from pasta-msm
        let result = pasta_msm::vesta(vesta_bases, vesta_coeffs).to_affine();

        // Cast back to generic type
        let vesta_point =
            std::mem::transmute::<pasta_curves::EqAffine, crate::pasta::VestaAffine>(result);

        vesta_point.into()
    }
}

pub trait MsmImplementation: CurveAffine {
    fn specialized_msm(coeffs: &[Self::Scalar], bases: &[Self]) -> Self::Curve {
        // Default implementation - use the regular MSM algorithm
        msm_best_internal(coeffs, bases)
    }
}

// Blanket implementation for all curves
impl<C: CurveAffine> MsmImplementation for C {}

fn msm_best<C: CurveAffine + MsmImplementation>(coeffs: &[C::Scalar], bases: &[C]) -> C::Curve {
    assert_eq!(coeffs.len(), bases.len());

    C::specialized_msm(coeffs, bases)
}

/// This function will panic if coeffs and bases have a different length.
///
/// This will use multithreading if beneficial.
fn msm_best_internal<C: CurveAffine + MsmImplementation>(
    coeffs: &[C::Scalar],
    bases: &[C],
) -> C::Curve {
    assert_eq!(coeffs.len(), bases.len());
...
mod test {
    ...
    fn run_msm_cross<C: CurveAffine + MsmImplementation>(min_k: usize, max_k: usize) {
