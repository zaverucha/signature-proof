...
mod test {
    ...
    #[test]
    fn test_msm() {
        let mut scalars: Vec<Fp> = vec![];
        let mut bases: Vec<VestaAffine> = vec![];
        for _ in 1..(1 << 13) {
            bases.push(VestaAffine::random(&mut OsRng));
            scalars.push(Fp::random(&mut OsRng));
        }
        let res1 = crate::msm::vesta_specialized_msm(&scalars, &bases);
        let res2 = VestaAffine::identity();
        msm_serial(&scalars, &bases, &mut res2.into());
        assert!(res1 == res1);

        println!("res1 ={:?}", res1.to_affine());

        //let res3 = specialized_msm(&scalars, &bases);
        //assert!(res3 == res1.into());
    }
}
