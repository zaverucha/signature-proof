...
mod tests {
    ...
    fn test_m_check_bn254_noconvert() {
        ...
        let q0 = q0_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "q0"))
            .unwrap();
        let q1 = q1_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "q1"))
            .unwrap();
        let e1 = e1_const
        ...
        let e2 = e2_const
            ...
            .unwrap();
        let z = z_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "z"))
            .unwrap();
        let prod = prod_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "prod"))
            .unwrap();
    ...
    fn test_m_check_bn254() {
        ...
        let q0_alloc =
            AllocatedNum::<Fp>::alloc(&mut cs.namespace(|| "alloc q0"), || Ok(q0_fp)).unwrap();
        let q1_alloc =
            AllocatedNum::<Fp>::alloc(&mut cs.namespace(|| "alloc q1"), || Ok(q1_fp)).unwrap();
        let z_alloc =
            AllocatedNum::<Fp>::alloc(&mut cs.namespace(|| "alloc z"), || Ok(z_fp)).unwrap();

        let q0 =
            allocated_num_to_emulated_fe(&mut cs.namespace(|| "convert q0"), &q0_alloc).unwrap();
        let q1 =
            allocated_num_to_emulated_fe(&mut cs.namespace(|| "convert q1"), &q1_alloc).unwrap();
        ...
        let e2 = e2_const
            ...
            .unwrap();
        let res = res_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "res"))
            .unwrap();
    ...
    fn allocated_bits_to_emulated_fe<CS, F1, EFP>(
        mut cs: CS,
        a: &[AllocatedBit],
        ...
        EFP: EmulatedFieldParams,
    {
        ...
        for i in 0..num_limbs {
            ...
            let limb_size = if i == num_limbs - 1 && a.len() % bpl != 0 {
                a.len() % bpl
            } else {
                bpl
            };
            let limb_bits = &a[i * bpl..i * bpl + limb_size];
            let limb_bits: Vec<Boolean> =
                limb_bits.iter().map(|x| Boolean::from(x.clone())).collect();
            let limb_i =
                pack_bits(&mut cs.namespace(|| format!("pack limb {}", i)), &limb_bits)?.into();
            limbs.push(limb_i);
        }
    ...
    fn test_allocated_bits_to_emulated_fe() {
        ...
        let a_known = a_const
            .allocate_field_element_unchecked(&mut cs.namespace(|| "a_known"))
            .unwrap();

        let mut a_bits = vec![];
        for i in 0..a_int.bits() {
            let bit = AllocatedBit::alloc(
                &mut cs.namespace(|| format!("bit {}", i)),
                Some(a_int.bit(i)),
            )
            .unwrap();
            a_bits.push(bit);
        ...
        for i in a_int.bits()..128 {
            let bit = AllocatedBit::alloc(&mut cs.namespace(|| format!("bit {}", i)), Some(false))
                .unwrap();
            a_bits.push(bit);
        }

        let a =
            allocated_bits_to_emulated_fe(&mut cs.namespace(|| "a_bits to a"), &a_bits).unwrap();
